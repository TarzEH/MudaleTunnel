...
class MudaleTunnelUI:
    ...
    def install_chocolatey(self):
        ...
        if approval == 'y':
            install_command = '''Set-ExecutionPolicy Bypass -Scope Process -Force; irm "https://community.chocolatey.org/install.ps1") | iex'''
            # subprocess.run(["start-proce", "-Command", install_command], check=True)
            subprocess.run([
                        "powershell", 
                        "Start-Process", 
                        "powershell", 
